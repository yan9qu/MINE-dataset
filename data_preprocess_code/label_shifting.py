...
def parser_args():
    ...
    parser.add_argument('--output', metavar='DIR',
                        default='/data/sqhy_model/uncertainty/mAP/uncertainty/uncertainty(q2l+q2l+cross(n+3+1))-1/test-1/Q2L/cutout-H',
                        help='path to output folder')
    parser.add_argument('--mode', default=1, type=int,
                        help='1: eff+ineff, 2:eff+eff, 3:ineff+ineff')
    parser.add_argument('--resume',
                        default="/data/sqhy_model/uncertainty/mAP/single/Q2L-1/model_best.pth.tar",
                        type=str, metavar='PATH',
                        help='path to latest checkpoint (default: none)')
    # /data/sqhy_model/uncertainty/mAP/single/Q2L-1/
    # /data/sqhy_model/uncertainty/mAP/uncertainty/uncertainty(q2l+q2l+iter(contrast+13feat)-4-50ep/  总
    # /data/sqhy_model/uncertainty/mAP/uncertainty/uncertainty(q2l+q2l+cross(n+3+1))-1/   logit
    # /data/sqhy_model/uncertainty/mAP/uncertainty/uncertainty(q2l+q2l+0.005feat)-3/  feature
...
def validate(val_loader, model, criterion, args, logger):
    ...
    with torch.no_grad():
        ...
        for i, (path, images, target) in enumerate(val_loader):
            ...
            saved_data.append(path)
            saved_data.append(entropy.detach().cpu())
        ...
        np.savetxt(os.path.join(args.output, saved_name), saved_data,fmt='%s')
        if dist.get_world_size() > 1:
            dist.barrier()

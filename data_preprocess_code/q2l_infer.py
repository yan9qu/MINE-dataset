...
def parser_args():
    ...
    parser.add_argument('--output', metavar='DIR',
                        default='/data/sqhy_model/MMintention/image_feature',
                        help='path to output folder')
    parser.add_argument('--resume',
                        default="/data/sqhy_model/MMintention/model_best.pth.tar",
    ...
    parser.add_argument('-a', '--arch', metavar='ARCH', default='VIT-img1k-244',
                        choices=available_models,
                        help='model architecture: ' + ' | '.join(available_models) +
                             ' (default: VIT-img1k-244)')
...
def main_worker(args, logger):
    ...
    if args.evaluate:
        mAP = validate(val_loader, model, criterion, args, logger)
        return


@torch.no_grad()
def validate(val_loader, model, criterion, args, logger):
    fr = open('/data/sqhy_model/MMintention/image_feature/image_features.pkl', 'rb')
    ...
    with torch.no_grad():
        ...
        save_features =[]
        dict = {}
        for i, (images, target, images_path) in enumerate(val_loader):
        #     单独存每张图片
        ...
        #             feature = model(img)
        #             img_path = images_path[j]
        #             img_path = img_path[0].split("/")
        #             if 'jpg' not in img_path[-1]:
        #                 feature = torch.zeros(1, 768)
        #             dict = {'img_path': images_path[j], 'img_feature': feature}
        #             save_features.append(dict)
        # filename1 = os.path.join(args.output, 'image_features.pkl')
        # with open(filename1, 'wb') as f1:
        #     # json.dump(save_features, f1)
        #     # saved_name = 'saved_image_features.{}.txt'.format(dist.get_rank())
        #     # np.save(os.path.join(args.output, saved_name), save_features)
        #     pickle.dump(save_features, f1, -1)
        #     target = target.cuda(non_blocking=True)
            middle_dict= []
            for j in range(len(images)):
                img = images[j]
                ...
                with torch.cuda.amp.autocast(enabled=args.amp):
                    ...
                    img_path = images_path[j]
                    img_path = img_path[0].split("/")
                    if '.' not in img_path[-1]:
                        final_feature = torch.zeros(768).numpy()
                    else:
                        middle_dict.append(feature.cpu().numpy())
            if '.' in img_path[-1]:
                middle_dict = numpy.array(middle_dict)
                middle1_dict = numpy.squeeze(middle_dict)
                if len(images) == 1:
                    final_feature = middle1_dict
                else:
                    final_feature = middle1_dict.mean(axis=0)

            if '.' in img_path[-1]:
                dict[img_path[-2]] = final_feature
            else:
                dict[img_path[-1]]= final_feature
            # save_features.append(dict)
            filename1 = os.path.join(args.output, 'image_features.pkl')
            with open(filename1, 'wb') as f1:
                # json.dump(save_features, f1)

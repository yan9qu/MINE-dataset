...
def parser_args():
    ...
    parser.add_argument('--output', metavar='DIR',
                        default='/data/sqhy_model/uncertainty/mAP/final_exper/uncertainty(q2l+q2l+(contrast+13feat))-6-50ep/testtttttttttttttt',
                        help='path to output folder')
    parser.add_argument('--mode', default=1, type=int,
                        help='1: eff+ineff, 2:eff+eff, 3:ineff+ineff')
    parser.add_argument('--resume',
                        default="/data/sqhy_model/uncertainty/mAP/final_exper/uncertainty(q2l+q2l+(contrast+13feat))-6-50ep/model_best.pth.tar",
...
def validate(val_loader, model, criterion, args, logger):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            _item =  target.detach().cpu()
            if i == 0:
                save = _item
            else:
                save = torch.cat((save, _item),0)
        ...
        bb = 1
        ww = torch.sum(save,0)
        saved_data = torch.cat(saved_data, 1)

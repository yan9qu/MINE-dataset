...
from data_utils.get_dataset_text import get_datasets
...
def parser_args():
    ...
    parser.add_argument('--output', metavar='DIR',
                        default='/data/sqhy_model/MMintention/text_feature',
...
def main_worker(args, logger):
    ...
    if args.evaluate:
        mAP = validate(val_loader, tokenizer, model, criterion, args, logger)
        return


@torch.no_grad()
def validate(val_loader, tokenizer, model, criterion, args, logger):
    # fr = open('/data/sqhy_model/MMintention/text_feature/text_features111111.pkl', 'rb')
    ...
    with torch.no_grad():
        save_features =[]
        dict= {}
        for i, (text_path, target) in enumerate(val_loader):
            # target = target.cuda(non_blocking=True)
            id_path = text_path[0].split('/')
            if '.txt' in text_path[0]:
                with open(text_path[0], "r") as f:
                    ...
                    sentence1_embedding = outputs.pooler_output  # 整个句子的句子向量
                    sentence1_embedding = sentence1_embedding.cpu().numpy()
                    sentence_embedding = numpy.squeeze(sentence1_embedding)
                    id = id_path[-2]
                    # input = tokenizer(text)
                    # feature = model(**input)
            else:
                sentence_embedding = torch.zeros(768).numpy()
                id = id_path[-1]
            dict[id] = sentence_embedding
            # save_features.append(dict)
        filename1 = os.path.join(args.output, 'text_features.pkl')
